...
#define GROUPSIZE                                                              \
  8 // 8 is the maximum number of counted events ( without multiplexing on my
    // Ivybridge -- HT switched off )
...
struct PFMWatch {
  ...
  uint64_t count[GROUPSIZE], startvalues[3 * GROUPSIZE],
      stopvalues[3 * GROUPSIZE];
  ...
  void Stop() {
    ...
    for (unsigned int i = 0; i < GROUPSIZE; ++i) {
      ...
      if (stopvalues[3 * i + 2])
        count[i] = (uint64_t)(
            (double)(stopvalues[3 * i + 0] - startvalues[3 * i + 0]) *
