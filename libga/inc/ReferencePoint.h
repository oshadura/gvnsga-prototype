...
public:
	...
	void GenerateRecursivelyRP(std::vector<ReferencePoint> *fRP, ReferencePoint *p, size_t fNumberObjects, size_t fLeft, size_t fTotal, size_t fElement);
	void GenerateRP(std::vector<ReferencePoint> *fRP , size_t fSize, const std::vector<std::size_t> &p);
	void AssociateRP(std::vector<ReferencePoint> *fRP, const Population<Double_t> &pop, std::vector<std::vector<Int_t>> &fFront);
	Double_t PerpedicularDistance(const std::vector<double> &fDirection, const std::vector<double> &fPoint);
