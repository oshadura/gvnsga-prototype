...
public:
  ...
  double ASF(const std::vector<double> &objs,
             const std::vector<double> &weight);

  // GuassianElimination(): used to calculate the hyperplane
  void GuassianElimination(std::vector<double> *px,
                           std::vector<std::vector<double>> A,
                           const std::vector<double> &b);

  // PerpendicularDistance(): calculate the perpendicular distance from a point
  // to a line
  double PerpendicularDistance(const std::vector<double> &direction,
  ...
  //
  // Given the number of objectives (M) and the number of divisions (p),
  // generate the set of
  // reference points. Check Section IV-B and equation (3) in the original
  // paper.

  void GenerateReferencePoints(std::vector<ReferencePoint> *rps, std::size_t M,
  ...
  class CPopulation;
  void Associate(std::vector<ReferencePoint> *prps, const Population &pop,
  ...
  void EnvironmentalSelection(
      Population *pnext,               // population in the next generation
