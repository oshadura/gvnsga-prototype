...
void ReferencePoint::GenerateRecursivelyRP(std::vector<ReferencePoint> *fRP, ReferencePoint *fP, size_t fNumberObjects, size_t fLeft, size_t fTotal, size_t fElement){
	if(fElement == fNumberObjects - 1){
		...
		for (int i = 0; i < fLeft; ++i){
			...
			GenerateRecursivelyRP(fRP, fP, fNumberObjects, (fLeft - i), fTotal, (fElement + 1));
		}
	}
}

void ReferencePoint::GenerateRP(std::vector<ReferencePoint> *fRP , size_t fSize, const std::vector<std::size_t> &p){
	...
	if (p.size() > 1)
	{
		...
		for (int i = 0; i < fInsideReferencePoint.size(); ++i)
		{
			...
			{
				fInsideReferencePoint[i].Position()[j] = (fCenter + fInsideReferencePoint[i].Position()[j])/2;
			}
...
void ReferencePoint::AssociateRP(std::vector<ReferencePoint> *fRP, const Population<Double_t> &pop, std::vector<std::vector<Int_t>> &fFront){
	...
	for (int t = 0; t < fFront.size(); ++t)
	{
		for (int i = 0; i < fFront[t].size(); ++i)
		{
			...
			{	
				...
				point.push_back(1);
				Double_t d = PerpedicularDistance(fRPCopy[r].Position(),  point /*pop[fFront[t][i] ].conv_objs()*/);
				if (d < fMinDist)
				{
					fMinDist = d;
			...
			if (t + 1 != fFront.size()){
				...
				fRPCopy[fMinReferencePoint].AddPotentialMemberRP(fFront[t][i], fMinDist);
			}
		}
	}
}
...
// ---------------------------------------------------------------------
Double_t ReferencePoint::PerpedicularDistance(const std::vector<double> &fDirection, const std::vector<double> &fPoint)
{
...
// ----------------------------------------------------------------------
std::vector<Double_t> TranslateObjectives(Population<Double_t> *pop, const std::vector<std::vector<Int_t>> &fFront)
{
	Population &p = *pop;
...
// ----------------------------------------------------------------------
void FindExtremePoints(std::vector<size_t> *extreme_points, const CPopulation &pop, const CNondominatedSort::TFronts &fronts)
{
